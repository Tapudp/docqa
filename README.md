...
- **Document ingestion** — drag-and-drop PDFs, Word docs, spreadsheets, images. Background workers parse and index every page automatically. Maximum file size is **200 MB per file** (regular upload) or **10 GB per file** (admin bulk upload).
...
- **Bulk upload** — admin can upload dozens of files (or a ZIP archive) to any workspace in one go. Each file is queued and processed independently.
...
See [DEPLOY_K8S.md](./DEPLOY_K8S.md) for the full guide. All services run as Kubernetes workloads. The repo includes a single root `Dockerfile` with named build targets:

```bash
# Build both images from the repo root
docker build --target api      -t ghcr.io/tapudp/docqa-api:latest .
docker build --target frontend \
  --build-arg NEXT_PUBLIC_API_URL=http://api.your-cluster \
...
│   │   ├── parser/       # Document parsers (PDF, DOCX, XLSX, PPTX)
│   │   ├── retrieval/    # Hybrid search (BM25 + vector + RRF)
│   │   ├── schemas/      # Pydantic schemas
│   │   ├── storage/      # MinIO client
│   │   └── worker/       # Celery tasks (parse, chunk, index)
│   └── Dockerfile        # Used by docker-compose (local dev)
├── frontend/
│   ├── app/
...
│   │   └── login/
│   ├── components/
│   │   ├── chat/              # ChatMessage, CitationBadge, TypingIndicator
│   │   ├── documents/         # DocumentCard, PDFViewer, UploadZone, BulkUploadModal
│   │   └── layout/            # Sidebar, PDFViewerPanel
│   └── lib/                   # Zustand store, API client, types
├── Dockerfile             # Root multi-target build (--target api / --target frontend)
├── docker-compose.yml     # Local dev stack
├── BUILD_PHASES.md        # Phase-by-phase build history
├── DEPLOY.md              # Docker Compose setup guide
...
| Limit | Value |
|-------|-------|
| Max file size (regular upload) | 200 MB per file |
| Max file size (admin bulk upload) | 10 GB per file |
...
| 10 | Admin bulk upload — ZIP extraction, 10 GB limit, per-file progress |
